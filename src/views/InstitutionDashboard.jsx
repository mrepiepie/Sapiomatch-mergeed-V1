...
export default function InstitutionDashboard({ currentUser, alert, appliedCourses = [], onRefreshApplications }) {
  const [credits, setCredits] = useState(currentUser?.credits !== undefined ? currentUser.credits : 1240);
  ...
  const handleTopup = async (amount) => {
    try {
      const instId = currentUser?.id || currentUser?.email || 'uni_1';
      const response = await fetch(`/api/institutions/${instId}/credits?amount=${amount}`, {
        method: 'POST'
      });
      if (response.ok) {
        const data = await response.json();
        setCredits(data.new_balance);
        alert(`Success! Credited AED ${amount} to your lead generation balance. Current balance is now ${data.new_balance} credits.`);
      } else {
        const err = await response.json();
        alert(err.error || "Failed to credit top-up on database.");
      }
    } catch (err) {
      console.error(err);
      alert("Error connecting to database credits service.");
    }
  };
