...
export default function AdminDashboard({ 
  ...
  const [selectedInstId, setSelectedInstId] = useState('uni_1');
  const [optEnglish, setOptEnglish] = useState(false);
  const [optDocs, setOptDocs] = useState(false);
  const [optStatement, setOptStatement] = useState(false);
  ...
  const loadTemplate = async (instId) => {
    try {
      const res = await fetch(`/api/forms/templates/${instId}`);
      if (res.ok) {
        const data = await res.json();
        const opts = data.optional_sections || [];
        setOptEnglish(opts.includes("English / Language Proficiency"));
        setOptDocs(opts.includes("Documents Upload"));
        setOptStatement(opts.includes("Personal Statement"));
      }
    } catch (err) {
  ...
  const handleSaveTemplate = async () => {
    try {
      ...
      if (optEnglish) optional_sections.push("English / Language Proficiency");
      if (optDocs) optional_sections.push("Documents Upload");
      if (optStatement) optional_sections.push("Personal Statement");

      const res = await fetch(`/api/forms/templates/${selectedInstId}/update`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ optional_sections })
      });

      if (res.ok) {
        alert("Success! University application form template updated persistently.");
      } else {
        alert("Failed to update template.");
      }
    } catch (err) {
      console.error(err);
      alert("Error connecting to template update service.");
    }
  };

  useEffect(() => {
    if (activeTab === 'formcustomizer' && selectedInstId) {
      loadTemplate(selectedInstId);
    }
  }, [activeTab, selectedInstId]);

  const handleAdjustCredits = async (user) => {
    const amountStr = prompt(`Enter change in credits for ${user.name} (e.g. +100 or -50):`, "0");
    if (!amountStr) return;
    const amount = parseFloat(amountStr);
    if (isNaN(amount) || amount === 0) {
      alert("Invalid credit change amount.");
      return;
    }
    ...
    try {
      ...
      const res = await fetch('/api/users/profile/update', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: user.id, credits: newCredits })
      });

      if (res.ok) {
        alert(`Success! Updated credits for ${user.name}. New balance: ${newCredits}`);
        fetchUsers();
      } else {
        const err = await res.json();
      ...
      console.error(err);
      alert("Error connecting to credits service.");
    }
  };
      ...
      <div className="glass-card" style={{ padding: '0', overflow: 'hidden' }}>
        <div style={{ display: 'flex', borderBottom: '1px solid var(--card-border)', background: 'rgba(255, 255, 255, 0.01)', flexWrap: 'wrap' }}>
          {[
            ...
            { id: 'formcustomizer', label: 'Customize Application Forms' },
        ...
        <div style={{ padding: '24px' }}>
          ...
          {/* Customize Application Forms Tab */}
          {activeTab === 'formcustomizer' && (
            <div style={{ maxWidth: '600px' }}>
              <h3 style={{ fontSize: '18px', color: 'white', fontWeight: 600, marginBottom: '8px' }}>
                Application Form Customizer
              </h3>
              <p style={{ color: 'var(--text-muted)', fontSize: '13px', marginBottom: '20px' }}>
                Toggle which optional sections are required for candidates applying to each partner institution.
              </p>

              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', background: 'rgba(255,255,255,0.01)', padding: '20px', borderRadius: '8px', border: '1px solid var(--card-border)', marginBottom: '20px' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '13px', color: 'var(--text-muted)', marginBottom: '8px' }}>Select Partner University</label>
                  <select
                  ...
                  >
                    {/* Fallback to default lists if institutions is empty */}
                    {institutions.length > 0 ? (
                      ...
                      <>
                        <option value="uni_1">American University of Sharjah</option>
                        <option value="uni_2">University of Birmingham Dubai</option>
                      </>
                    )}
                  </select>
                </div>
                ...
                <div style={{ marginTop: '8px' }}>
                  <label style={{ display: 'block', fontSize: '13px', color: 'var(--text-muted)', marginBottom: '12px' }}>
                    Configure Optional Form Fields
                  </label>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '13.5px', color: 'white', cursor: 'pointer' }}>
                      <input
                        type="checkbox"
                        checked={optEnglish}
                        onChange={(e) => setOptEnglish(e.target.checked)}
                        style={{ width: '16px', height: '16px', accentColor: 'var(--secondary)' }}
                      />
                      <span>English / Language Proficiency (IELTS/TOEFL input)</span>
                    </label>
                    ...
                    <label style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '13.5px', color: 'white', cursor: 'pointer' }}>
                      <input
                        type="checkbox"
                        checked={optDocs}
                        onChange={(e) => setOptDocs(e.target.checked)}
                        style={{ width: '16px', height: '16px', accentColor: 'var(--secondary)' }}
                      />
                      <span>Documents Upload (CV/Resume attachment)</span>
                    </label>

                    <label style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '13.5px', color: 'white', cursor: 'pointer' }}>
                      <input
                        type="checkbox"
                        checked={optStatement}
                        onChange={(e) => setOptStatement(e.target.checked)}
                        style={{ width: '16px', height: '16px', accentColor: 'var(--secondary)' }}
                      />
                      <span>Personal Statement (SOP Essay input)</span>
                    </label>
                  </div>
                </div>
              </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '14px' }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid var(--card-border)', color: 'var(--text-muted)' }}>
                    ...
                    <th style={{ padding: '12px' }}>Credits Balance</th>
                ...
                <tbody>
                  {usersList.map(user => (
                    <tr key={user.id} style={{ borderBottom: '1px solid var(--card-border)' }}>
                      ...
                      <td style={{ padding: '14px 12px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          <span style={{ fontWeight: 700, color: 'var(--secondary)' }}>{user.credits !== undefined ? user.credits : 0}</span>
                          <button
                            className="btn-premium-outline"
                            onClick={() => handleAdjustCredits(user)}
                            style={{ padding: '2px 6px', fontSize: '10px', height: 'auto', minHeight: 'auto', display: 'inline-flex' }}
                          >
                            Adjust
                          </button>
                        </div>
