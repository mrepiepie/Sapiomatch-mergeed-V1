...
export default function Results({ setView, answers, bookmarks = [], toggleBookmark, applyForCourse, appliedCourses = [], alert, currentUser, plan = 'Standard', onUpdateMembership }) {
  ...
  const downloadReport = () => {
    let text = `====================================================\n`;
    text += `               LEARNOVA AI MATCH REPORT             \n`;
    text += `====================================================\n\n`;
    text += `Date: ${new Date().toLocaleDateString()}\n`;
    text += `Candidate: ${currentUser?.name || 'Verified Candidate'}\n`;
    text += `Academic Background: ${answers.education || "Bachelor's degree"}\n`;
    text += `Field of Interest: ${answers.field || "Technology & AI"}\n`;
    text += `Target Goal: ${answers.goal || "Get Promoted"}\n`;
    text += `Budget Level: ${answers.budget ? 'Low budget' : 'Affordable'}\n\n`;
    text += `----------------------------------------------------\n`;
    text += `                    TOP MATCHES                     \n`;
    text += `----------------------------------------------------\n\n`;
    
    matches.slice(0, 3).forEach((match, idx) => {
      text += `${idx + 1}. ${match.title} (${match.matchScore}% Match)\n`;
      text += `   Institution: ${getInstitutionName(match.institutionId)}\n`;
      text += `   Annual Fee: ${match.fee}\n`;
      text += `   Format: ${match.format} | Duration: ${match.duration}\n`;
      text += `   Key Pros:\n`;
    ...
    text += `====================================================\n`;
    text += `       Generated by Learnova Smart Match Engine      \n`;
    text += `====================================================\n`;
    ...
    a.href = url;
    a.download = `Learnova_AI_Match_Report.txt`;
    ...
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '40px 24px' }} className="page-fade-enter">
      ...
      <div className="spotlight-card" onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} style={{ padding: '20px', marginBottom: '40px', '--spotlight-color': 'rgba(180, 83, 9, 0.08)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', borderBottom: '1px solid var(--card-border)', paddingBottom: '10px', position: 'relative', zIndex: 2 }}>
          ...
          <button onClick={downloadReport} className="btn-premium" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '6px 12px', fontSize: '12px' }}>
