...
export default function Questionnaire({ setView, answers, setAnswers, completedQuiz, setCompletedQuiz, currentUser, questions, setQuestions }) {
  ...
  const handleBack = () => {
    if (currentStep > 1) {
      ...
      setCurrentStep(0); // Back to welcome screen
    }
  };
  ...
  const handleTouchEnd = () => {
    ...
    const isRightSwipe = distance < -50; // swipe right to go back
    if (isRightSwipe) {
      handleBack();
    }
  };
  ...
  const handleOptionSelect = (option) => {
    ...
    setTimeout(() => {
      ...
      let nextStep = null;
      for (let step = currentStep + 1; step <= activeQuestions.length; step++) {
        const k = getAnswerKey(step);
        if (!updatedAnswers[k]) {
          nextStep = step;
          break;
        }
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTimeout(() => {
          setExtractedData(data.profile || data);
          setIsScanning(false);
        }, 300);
      } else {
      ...
      clearInterval(interval);
      alert("Error connecting to parser service.");
      setIsScanning(false);
    }
  };
  ...
  const confirmExtractedData = () => {
    ...
    // Find the first unanswered step starting from step 1
    let firstUnansweredStep = 1;
    for (let step = 1; step <= activeQuestions.length; step++) {
      const k = getAnswerKey(step);
      if (!updatedAnswers[k]) {
        firstUnansweredStep = step;
        break;
      }
    }
      ...
      {currentUser?.role === 'Admin' && !isAdminPreview ? (
          ...
          <div className="glass-card anim-glow" style={{ padding: '40px', textAlign: 'center', border: '1px solid var(--card-border)', background: 'rgba(255, 255, 255, 0.01)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', maxWidth: '380px', margin: '0 auto' }}>
              ...
              >
                ...
                <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Upload PDF or DOCX. AI instantly extracts your profile.</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: '24px', alignItems: 'center' }} className="page-fade-enter">
          ...
          {isCalculating ? (
            ...
            <div 
              className="glass-card anim-glow" 
                ...
                {aiState === 'thinking' ? (
                  ...
                  currentStep <= activeQuestions.length && (
                    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '32px' }} className="page-fade-enter">
                      ...
                      {/* MCQ / Dropdown Options Container */}
                      {getAnswerKey(currentStep) === 'experience' ? (
                        <div style={{ width: '100%', maxWidth: '400px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                          <select
                            value={answers[getAnswerKey(currentStep)] || ''}
                            onChange={(e) => {
                          ...
                          {activeQuestions[currentStep - 1].options.map((opt, idx) => {
                            ...
                            return (
                              <button
                                ...
                                className={isSelected ? "btn-premium" : "btn-premium-outline"}
                                style={{
                                  padding: '16px 20px',
                                  fontSize: '14px',
                                  fontWeight: 500,
                                  justifyContent: 'center',
                                  background: isSelected ? 'var(--secondary)' : 'rgba(255, 255, 255, 0.01)',
                      ...
                      {/* Next Step / Skip review button for pre-filled data */}
                      {answers[getAnswerKey(currentStep)] && (
                        <button
                          className="btn-premium"
                          onClick={() => {
                            setAiState('thinking');
                        ...
                        >
                          {currentStep === activeQuestions.length ? "Finish & View Matches" : "Next Step"}
