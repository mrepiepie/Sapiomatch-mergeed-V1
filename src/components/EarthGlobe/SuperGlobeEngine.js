  ...
  bindEvents() {
    ...
    this.canvas.addEventListener("pointermove", (event) => {
      ...
      if (this.isDragging) {
        ...
        this.earthGroup.rotation.y += dx * 0.008;
        this.earthGroup.rotation.x += dy * 0.006;
        this.earthGroup.rotation.x = THREE.MathUtils.clamp(this.earthGroup.rotation.x, -0.9, 0.9);

        this.dragVelocity = { x: dx * 0.001, y: dy * 0.0006 };
        this.lastPointer = { x: event.clientX, y: event.clientY };
      }
    ...
    if (contentArea) {
      contentArea.innerHTML = `
        ...
        <button
          ...
          data-globe-destination="true"
          data-country="${this.escapeHTML(key)}"
          style="background: rgba(34, 211, 238, 0.1); border-color: rgba(34, 211, 238, 0.25); color: white; margin-bottom: 20px; text-align: left; width: 100%; border-radius: 8px; padding: 12px;"
        >
          <span class="directory-uni-card-topline">
            <span class="directory-uni-card-name" style="color: #22d3ee; display: flex; align-items: center; gap: 6px; font-weight: 600;">
              <i data-lucide="compass" style="width:14px; height:14px;"></i> Explore ${this.escapeHTML(hub.name)} Portal
            </span>
          </span>
          <span class="directory-uni-card-footer" style="margin-top: 8px; display: flex; justify-content: space-between; align-items: center; width: 100%;">
            <span style="font-size: 11px; color: var(--muted-foreground);">Visa rules, fees, & academic guidelines</span>
            <i data-lucide="arrow-right" style="width:13px; color: #22d3ee;"></i>
          </span>
        </button>
