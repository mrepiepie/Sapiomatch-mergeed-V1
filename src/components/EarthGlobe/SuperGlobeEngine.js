...
export default class SuperGlobeEngine {
  constructor(canvasId, sidebarId) {
  ...
  bindEvents() {
    ...
    const closeBtn = document.getElementById("globe-sidebar-close-btn");
    if (closeBtn) {
  ...
  openSidebar(key) {
    ...
    const hub = this.hubs[key];
    const heading = document.getElementById("globe-sidebar-title-heading");
    if (heading) {
      heading.innerHTML = `
    ...
    const contentArea = document.getElementById("globe-sidebar-content-area");
    if (contentArea) {
      contentArea.innerHTML = `
        ...
        ${hub.universities.map((u) => {
          const link = this.getUniversityLink(u);
          return `
            <div class="directory-uni-card" style="margin-bottom:12px;">
              <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:8px; margin-bottom:6px;">
                <div class="directory-uni-card-name" style="font-size:13px; font-weight:600; color:white; line-height:1.4;">${this.escapeHTML(u.name)}</div>
                <span class="badge" style="font-size:8px; padding:2px 5px; background:rgba(34,211,238,0.1); color:#22d3ee; border:1px solid rgba(34,211,238,0.25); border-radius:4px; flex-shrink:0;">Active</span>
              </div>
              <div class="directory-uni-card-meta" style="margin-bottom:12px;">
                <span style="font-size:11px; color:var(--muted-foreground); display:flex; align-items:center; gap:6px;"><i data-lucide="building" style="width:12px;"></i> ${this.escapeHTML(u.type || "University")}</span>
                <span style="font-size:11px; color:var(--muted-foreground); display:flex; align-items:center; gap:6px;"><i data-lucide="map-pin" style="width:12px;"></i> ${this.escapeHTML(u.location || hub.name)}</span>
                <span style="font-size:11px; color:#10b981; font-weight:500; display:flex; align-items:center; gap:6px;">
                  <span style="display:inline-block; width:5px; height:5px; background:#10b981; border-radius:50%; box-shadow:0 0 6px #10b981;"></span> Directory Listed
                </span>
              </div>
              <a href="${this.escapeHTML(link.href)}" ${link.attributes} class="btn btn-secondary directory-uni-card-btn" style="background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.08); color:white; font-size:11px; padding:6px 12px; width:100%;">
